    ...
    return (
        <div>
            ...
            <div className='relative'>
                ...
                {/* Overlay that covers the content outside the Navbar */}
                {isMenuOpen && (
                    <div
                        className='fixed inset-0 bg-black bg-opacity-30 z-40'
                        onClick={() => setIsMenuOpen(false)} // Close navbar when clicking on overlay
                    />
                )}
                ...
                <div
                    ref={menuRef}
                    className='bg-white sm:hidden shadow-md p-4 w-full absolute min-h-8/12 pb-16 z-50'
                    style={{ transform: 'translateY(-100%)' }}
                >
                    <select className='w-full mb-4 px-4 py-2 border border-gray-300 rounded-md'>
                        <option className='text-sm font-medium'>UtkarshDhairyaPanwar</option>
                    </select>
